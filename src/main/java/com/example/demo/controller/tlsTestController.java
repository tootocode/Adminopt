...
import com.example.demo.database.Query.LibQuery;
...
public class tlsTestController {
    ...
    @GetMapping("/tlstest")
    public String postdata(String name,String target,String desc,String date){  //获取前端的tls
    ...
    public List<tlstaskInfo> getdata(String name, String target, String tlsbug) throws SQLException {
        ...
        String sql2="select distinct host,taskdate,taskname from CurveSwapFinalTable where ";
    ...
    public List<tlstaskInfo> getInitdata() throws SQLException {
        ...
        LibQuery libQuery=new LibQuery(connection);
        for(tlstaskInfo bug:list){
            bug.lib=libQuery.queryRecord(bug.getTarget());
        }
        String sql2="select distinct host,taskdate,taskname from CurveSwapFinalTable where InvalidCurveSwap='1' or TwistCurveSwap='1'";
        List<tlstaskInfo> curveswaplist=tlstaskQuery.queryRecord(sql2,"curveswap");
        for(tlstaskInfo bug:curveswaplist){
            bug.lib=libQuery.queryRecord(bug.getTarget());
        }
        list.addAll(curveswaplist);
        connection.close();
        return list;
    }
}
